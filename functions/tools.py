...
class Tools:
    ...
    async def compress(self, dl, out):
        cmd = f'''{Var.FFMPEG} -i """{dl}"""  -preset ultrafast -c:v libx265 -crf 28 -map 0:v -c:a aac -metadata:s:v:0 title="[Anime Compass] : (This Episode)" -metadata:s:a:0 title="[Telegram: @Anime_Compass]" -metadata:s:a:1 title="[Telegram: @Anime_Compass]" -map 0:a -c:s copy -map 0:s? -ab 64k -vf "drawtext=fontfile=Imprima_Love.ttf:fontsize=27:fontcolor=white:bordercolor=black@0.50:x=w-tw-10:y=10:text=Anime Compass" """{out}""" -y'''
        process = await asyncio.create_subprocess_shell(
